...
func removeFromStartup(targetPath string) error {
	if runtime.GOOS == "windows" {
		return removeFromStartupWindows(targetPath)
	} else if runtime.GOOS == "linux" {
		return removeFromStartupLinux(targetPath)
	}
	return fmt.Errorf("unsupported operating system")
}

func removeFromStartupWindows(targetPath string) error {
	key, _, err := registry.CreateKey(registry.CURRENT_USER, `Software\Microsoft\Windows\CurrentVersion\Run`, registry.SET_VALUE)
	if err != nil {
		return err
	}
	...
	return key.DeleteValue(filepath.Base(targetPath))
}

func removeFromStartupLinux(targetPath string) error {
	home, err := os.UserHomeDir()
	if err != nil {
		return err
	}
